...
export async function GET(request:NextRequest){try{const query=request.nextUrl.searchParams,isAdmin=query.get("public")!=="1"&&verifyAdminSession(request);if(query.has("goal")&&!isAdmin)throw new PlanningInputError("Admin login required",401);if(query.has("goal")||query.get("overview")==="1")throw new PlanningInputError("Goals have been removed. Use club events instead.",410);return NextResponse.json(await readClubPlanning(isAdmin,{event:query.get("event")||undefined}),{headers});}catch(error){return failure(error);}}
