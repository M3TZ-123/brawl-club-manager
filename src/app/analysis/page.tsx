...
function AnalysisContent() {
  ...
  return <div className="space-y-5">
    ...
    <details className="rounded-lg border p-3"><summary className="cursor-pointer text-sm font-medium">{t("Filter battles")}{[context, mode, map, brawler].filter(Boolean).length > 0 && ` · ${t("{count} applied", { count: number([context, mode, map, brawler].filter(Boolean).length) })}`}</summary><div className="mt-3 grid grid-cols-2 gap-3 xl:grid-cols-4">{filters.map(filter => <div key={filter.id} className="min-w-0"><label htmlFor={filter.id} className="mb-1 block text-sm">{t(filter.label)}</label><select id={filter.id} value={filter.value} onChange={event => filter.set(event.target.value)} className="h-10 w-full min-w-0 rounded-md border bg-background px-2 text-sm">
    ...
    {(context || mode || map || brawler) && <Button variant="ghost" size="sm" className="mt-2" onClick={() => { setContext(""); setMode(""); setMap(""); setBrawler(""); }}>{t("Clear filters")}</Button>}</details>
    ...
    {data && <>
      <p className="text-xs text-muted-foreground">{t("Period")}: <LocalDate value={data.period.start} time /> — <LocalDate value={data.period.end} time /></p>
      <div className="grid grid-cols-2 gap-3">
        <Card><CardContent className="p-4"><p className="text-xs text-muted-foreground">{t("Member participations")}</p><p className="mt-1 text-2xl font-bold">{number(data.summary.observations)}</p></CardContent></Card>
        <Card><CardContent className="p-4"><p className="text-xs text-muted-foreground">{t("Decided-result win rate")}</p><p className="mt-1 text-2xl font-bold">{rate(data.summary.winRate)}</p><p className="mt-1 text-xs text-muted-foreground">{t("{wins} wins · {losses} losses", { wins: number(data.summary.wins), losses: number(data.summary.losses) })}</p></CardContent></Card>
      </div>
      {(data.limits.truncated || data.coverage.truncated) && <p className="text-sm text-amber-600 dark:text-amber-400">{t("Partial results — choose a shorter period.")}</p>}
      <details className="rounded-lg border p-3 text-sm">
        ...
        <div className="mt-3 space-y-2 text-xs text-muted-foreground"><p>{t("One record per club player in a battle; teammates count separately.")}</p><p>{t("{draws} draws · {unknown} unknown results", { draws: number(data.summary.draws), unknown: number(data.summary.unknownResults) })}</p><p>{t("Saved battle logs cannot guarantee a complete match history.")}</p><p>{t("{monitored} of {total} current players monitored for the full selected period.", { monitored: data.coverage.fullPeriodMonitoredPlayers, total: data.coverage.currentPlayers })}</p><p>{t("{stale} players have stale checks; {affected} players have possible gaps overlapping this period. Gap records are kept for {days} days.", { stale: data.coverage.stalePlayers, affected: data.coverage.affectedPlayers, days: data.coverage.retainedGapWindowDays })}</p><p>{t("Earliest saved battle")}: <LocalDate value={data.coverage.earliestBattleAt} time /></p><p>{t("Latest saved battle")}: <LocalDate value={data.coverage.latestBattleAt} time /></p>
          <dl className="grid gap-2 border-t pt-3 sm:grid-cols-2"><div><dt>{t("Recorded match duration")}</dt><dd className="font-medium text-foreground">{data.summary.durationObservations ? duration(data.summary.recordedDurationSeconds) : t("Unknown")}</dd></div><div><dt>{t("Average recorded duration")}</dt><dd>{data.summary.averageDurationSeconds == null ? t("Unknown") : duration(data.summary.averageDurationSeconds)}</dd></div></dl><p>{t("Reported for {count} participations. This is not time online.", { count: data.summary.durationObservations })}</p>
          {data.limits.facetCounts && (["modes", "maps", "brawlers"] as const).some(key => data.limits.facetCounts[key] > data.facets[key].length) && <p>{t("Some filter choices exceed the display limit. Shorten the period to see more choices.")}</p>}</div>
      </details>
