...
function CandidateCard({candidate,onChange,onError,criteria}:{candidate:Candidate;onChange:(value:Candidate)=>void;onError:(value:string)=>void;criteria?:ClubAdministration|null}) {
  ...
  return <article className="border rounded-lg bg-card p-5 space-y-4">
    ...
    {criteria&&<section className="space-y-1 text-sm"><h3 className="font-medium">{t("Transparent fit criteria")}</h3>{recruitmentFit({...candidate,manual_compatibility:manual},criteria).map(row=><p key={row.key}>{t(row.label)}: {t(fitLabels[row.status])}</p>)}<p className="text-xs text-muted-foreground">{t("Profile criteria use the last checked profile. Language and time compatibility are assessed manually; no commitment score is inferred.")}</p></section>}
    ...
    <div className="flex flex-wrap gap-2"><Button disabled={busy || (notes===candidate.notes && status===candidate.status && JSON.stringify(manual)===JSON.stringify(candidate.manual_compatibility??emptyCompatibility))} onClick={()=>void save()}>{t("Save")}</Button><Button variant="outline" disabled={busy} onClick={()=>void refresh()}>{t("Load profile")}</Button></div>
  </article>;
}
function RecruitmentWorkspace() {
  ...
  return <div className="space-y-6">
    ...
    <ClubAdministrationSettings onChange={setCriteria}/>
    ...
    <RecruitmentApplications onCandidate={()=>void reload()}/>
  </div>;
}
