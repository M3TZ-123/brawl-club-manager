...
export function PlanningWorkspace({isAdmin}:{isAdmin:boolean}){
  ...
  return <div className="space-y-5 min-w-0"><header><h1 className="text-3xl font-bold">{t("Club events")}</h1><p className="mt-2 text-sm text-muted-foreground">{t("Follow Mega Pig and organize club events.")}</p></header>
    ...
    {isAdmin&&!editing&&<MegaPigArchivePanel/>}
    ...
    {!isAdmin&&<details id="planning-admin" className="text-sm"><summary className="cursor-pointer text-primary">{t("Sign in to view Mega Pig and manage events")}</summary><AdminGate title="Manage club events" description="View Mega Pig history, organize teams and save event results."><span/></AdminGate></details>}
  </div>;
}
