...
export default function ReadinessPage() {
  ...
  return <LayoutWrapper><div className="space-y-5">
    ...
    {data && <>
      <div className="flex flex-wrap gap-x-5 gap-y-2 text-sm"><p><strong>{number(data.total)}</strong> {t("matching player–brawler records")}</p><p className="text-muted-foreground">{t("{count} current players have saved collection data.", { count: data.members.filter(member => member.brawlersObserved > 0).length })}</p></div>
      ...
      <div className="grid gap-3 lg:grid-cols-2">{data.rows.map(row => <Card key={`${row.player.tag}:${row.brawler.id}`}><CardContent className="space-y-3 p-4">
        ...
        <dl className="grid grid-cols-2 gap-3 text-sm"><div><dt className="text-xs text-muted-foreground">{t("Trophies")}</dt><dd>{unknownNumber(row.trophies)}</dd></div><div><dt className="text-xs text-muted-foreground">{t("Official highest trophies")}</dt><dd>{unknownNumber(row.highestTrophies)}</dd></div></dl>
