  ...
  return <LayoutWrapper><div className="space-y-5">
    ...
    {data && <>
      ...
      <div className="grid gap-3 lg:grid-cols-2">{data.rows.map(row => <Card key={`${row.player.tag}:${row.brawler.id}`}><CardContent className="space-y-3 p-4">
        ...
        <div className="flex flex-wrap items-center justify-between gap-2"><p className="text-sm">{t("Trophies")}: <strong>{unknownNumber(row.trophies)}</strong></p><p className="text-xs text-muted-foreground">{t("Saved on")}: <LocalDate value={row.observedAt} time /></p></div>
