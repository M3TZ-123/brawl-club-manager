...
export default function JoinPage(){
  ...
  return <LayoutWrapper><div className="mx-auto max-w-2xl space-y-5"><header><h1 className="text-2xl font-bold">{t("Apply to join the club")}</h1><p className="mt-2 text-sm text-muted-foreground">{t("Your application is private to club administrators. Entering a player tag does not verify account ownership. No invitation or message is sent automatically.")}</p></header>
    ...
    {info&&<section className="space-y-2 rounded-lg border p-4"><p dir="ltr">{info.club_tag}</p><h2 className="font-semibold">{t("Published recruitment criteria")}</h2><p>{t("Minimum trophies")}: {number(info.min_trophies)} · {t("Minimum power 11 brawlers")}: {number(info.min_power11)}</p>{info.min_ranked_points!=null&&<p>{t("Minimum ranked points")}: {number(info.min_ranked_points)}</p>}{info.language&&<p>{t("Preferred language")}: {info.language}</p>}{info.availability&&<p>{t("Preferred playing times and timezone")}: {info.availability}</p>}<p className="text-xs text-muted-foreground">{t("Meeting the criteria does not guarantee admission.")}</p></section>}
    {accepted?<p role="status" className="rounded border border-green-500 p-4">{t("Application received. Administrators can review it privately.")}</p>:info&&!info.recruitment_open?<p>{t("Applications are currently closed.")}</p>:info&&<form className="space-y-4" onSubmit={submit}>
      ...
      <label className="block text-sm">{t("Languages you use")}<Input maxLength={120} value={language} disabled={busy} onChange={e=>setLanguage(e.target.value)}/></label>
      <label className="block text-sm">{t("Usual playing times and timezone")}<Input maxLength={240} value={availability} disabled={busy} onChange={e=>setAvailability(e.target.value)}/></label>
      ...
      <p className="text-xs text-muted-foreground">{t("Do not include passwords, API keys or sensitive personal information. Rejected or archived applications may be removed after 90 days.")}</p>
