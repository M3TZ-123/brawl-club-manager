...
export function ClubStrength() {
  ...
  return <ClubIntelligencePanel title="Roster strength" resource={resource}>{data && <>
    ...
    <div className="space-y-2">{data.trophyBands.map(band => <div key={band.min} className="grid grid-cols-[7rem_1fr_2rem] items-center gap-2 text-xs"><span dir="ltr">{number(band.min)}{band.max === null ? "+" : `–${number(band.max - 1)}`}</span><div className="h-3 overflow-hidden rounded bg-muted"><div className="h-full bg-primary/60" style={{ width: `${data.members ? band.members / data.members * 100 : 0}%` }} /></div><span className="text-end">{number(band.members)}</span></div>)}</div>
    <details className="text-sm"><summary className="cursor-pointer text-primary">{t("Current Ranked distribution")}</summary><dl className="mt-2 grid grid-cols-2 gap-2">{data.ranks.map(row => <div key={row.rank || "unknown"} className="flex justify-between gap-2"><dt>{t(row.rank || "Unknown")}</dt><dd>{number(row.members)}</dd></div>)}</dl></details>
