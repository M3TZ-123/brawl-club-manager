...
export function ClubGoalCard({goal,onDetails}:{goal:PlanningGoal;onDetails?:()=>void}){
  ...
  return <article className="rounded-lg border bg-card p-4 space-y-3 min-w-0">
    ...
    <p className="text-xs text-muted-foreground">{t("Frozen roster: {count} members",{count:number(goal.cohortCount)})} · {dateTime(goal.startsAt)} – {dateTime(goal.endsAt)}</p>
    {goal.achievedAt&&<p className="text-sm text-emerald-600 dark:text-emerald-400">{t("Target first observed reached")}: {dateTime(goal.achievedAt)}</p>}
    {(goal.limited||goal.possibleGap)&&<p className="text-sm text-amber-700 dark:text-amber-400">{t(goal.metric==="participants"?(goal.possibleGap?"Possible battle gaps overlap this goal. Observed participation is a lower bound.":"Some battle observations are missing or stale. Participation may be undercounted."):"Limited observations. Missing or stale balances are not counted as zero.")}</p>}
    {goal.refreshedAt&&<p className="text-xs text-muted-foreground">{t("Progress checked")}: {dateTime(goal.refreshedAt)}</p>}
    {onDetails&&<Button type="button" variant="outline" size="sm" onClick={onDetails}>{t("Goal details")}</Button>}
  </article>;
}
export function ClubGoalsOverview(){
