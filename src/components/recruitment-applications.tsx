...
export function RecruitmentApplications({onCandidate}:{onCandidate:()=>void}){
  ...
  return <section className="space-y-4"><h2 className="text-xl font-bold">{t("Private applications")}</h2><p className="text-sm text-muted-foreground">{t("Review requests from visitors. Status changes do not send invitations.")}</p><div className="flex flex-wrap items-end gap-3"><label className="text-sm">{t("Application status")}<select aria-label={t("Application status")} value={filter} onChange={e=>setFilter(e.target.value)} className="ms-2 rounded border bg-background p-2">{["all",...statuses].map(value=><option key={value} value={value}>{t(value)}</option>)}</select></label><Button variant="outline" disabled={loading} onClick={()=>void load()}>{t("Reload list")}</Button></div>
