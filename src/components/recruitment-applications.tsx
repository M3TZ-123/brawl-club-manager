...
function ApplicationCard({application,onSaved,onCandidate}:{application:RecruitmentApplication;onSaved:(row:RecruitmentApplication)=>void;onCandidate:()=>void}){
  ...
  return <article className="space-y-3 rounded-lg border p-4"><p className="font-semibold" dir="ltr">{application.player_tag}</p><p className="text-xs text-muted-foreground"><LocalDate value={application.created_at} time/> · {t("Account ownership unverified")}</p><p className="whitespace-pre-wrap break-words">{application.message||t("No application message")}</p><p className="text-sm">{t("Languages you use")}: {application.language||t("Unknown")}</p><p className="text-sm">{t("Usual playing times and timezone")}: {application.availability||t("Unknown")}</p>
    ...
    <div className="flex flex-wrap gap-2"><Button disabled={busy} onClick={()=>void submit()}>{t("Save application review")}</Button><Button variant="outline" disabled={busy||added} onClick={()=>void submit(true)}>{t(added?"Added to watchlist":"Add to watchlist")}</Button>{draft&&draft.version!==application.version&&<Button variant="outline" disabled={busy} onClick={()=>{setDraft(null);setError("");}}>{t("Load saved review")}</Button>}</div>
  </article>;
}
...
export function RecruitmentApplications({onCandidate}:{onCandidate:()=>void}){
  ...
  return <section className="space-y-4 border-t pt-6"><h2 className="text-xl font-bold">{t("Private applications")}</h2><p className="text-sm text-muted-foreground">{t("These details were submitted by visitors and are unverified. Status changes do not contact or invite anyone.")}</p><select aria-label={t("Application status")} value={filter} onChange={e=>setFilter(e.target.value)} className="rounded border bg-background p-2">{["all",...statuses].map(value=><option key={value} value={value}>{t(value)}</option>)}</select><Button variant="outline" disabled={loading} onClick={()=>void load()}>{t("Reload list")}</Button>
