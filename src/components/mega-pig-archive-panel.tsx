...
const sourceBadge = (source: MegaPigObservationSummary["source"]) => <bdi dir="ltr" className="inline-block rounded border px-1.5 py-0.5 text-xs text-muted-foreground">{source}</bdi>;
...
export function MegaPigFinalizeForm({ cycle, onSaved, onReload }: { cycle: MegaPigArchiveCycle; onSaved: (id: string) => void; onReload: () => void }) {
  ...
  return <form onSubmit={submit} className="space-y-3"><p className="text-xs text-muted-foreground">{t("Confirm results from the game; leave unknown values blank.")}</p><fieldset disabled={busy} className="space-y-3">
...
export function MegaPigCycleSummary({ cycle, now }: { cycle: MegaPigArchiveCycle; now?: number }) {
  ...
  return <div className="space-y-2"><div className="flex flex-wrap gap-2 justify-between"><h3 className="font-semibold break-words">{cycle.title}</h3><span className="text-xs text-muted-foreground">{t(lifecycleLabels[progress.lifecycle])}</span></div><p className="text-xs text-muted-foreground">{dateTime(cycle.startsAt)} – {dateTime(cycle.endsAt)}</p>
    ...
    {cycle.capturePausedReason && <p className="text-xs text-amber-600 dark:text-amber-400">{t(cycle.capturePausedReason === "source_changed" ? "Collection paused: source changed. Review the new reading before continuing." : "Collection paused: source counters decreased.")}</p>}
  </div>;
}
...
export function MegaPigArchivePanel({ initialPlayerTag = "" }: { initialPlayerTag?: string } = {}) {
  ...
  const readingSummary = (observation: MegaPigObservationSummary) => <div className="space-y-2">
    {sourceBadge(observation.source)}
    <p className="text-sm">{t("Reported total wins")}: {number(observation.totalWins)}
      {observation.reportedBattlesPlayed != null ? <> · {t("Reported battles")}: {number(observation.reportedBattlesPlayed)}</>
        : observation.reportedPlayersPlayed != null ? <> · {t("Players reported by source")}: {number(observation.reportedPlayersPlayed)}</> : null}
    </p>
    <p className="text-xs text-muted-foreground">{t("Reading fetched")}: {dateTime(observation.lastFetchedAt)}</p>
  </div>;
  return <section id="mega-pig-history" className="rounded-lg border p-4 space-y-4 min-w-0 scroll-mt-20" aria-label={t("Mega Pig history")}>
    <header className="flex flex-wrap items-center justify-between gap-2"><h2 className="font-semibold">{t("Mega Pig history")}</h2><Button type="button" size="sm" variant="ghost" disabled={loading} onClick={() => void load()}>{t("Refresh history")}</Button></header>
    ...
    {view.mode === "cycles" && <>
      ...
      {data?.cycles?.length === 0 && <p className="text-sm text-muted-foreground">{t("No cycles yet. View counters in Saved readings.")}</p>}
    ...
    {view.mode === "cycle" && data?.cycle && <div className="space-y-4"><MegaPigCycleSummary cycle={data.cycle} now={now}/><p className="text-xs text-muted-foreground">{t(data.cycle.finalizedAt || now >= Date.parse(data.cycle.endsAt) ? "Collection ended" : !data.cycle.captureEnabled ? "Automatic collection off" : now < Date.parse(data.cycle.startsAt) ? "Automatic collection scheduled" : "Automatic collection enabled")} · {t("Last captured reading")}: {dateTime(data.cycle.lastCapturedAt)}</p>
      ...
      <h3 className="font-medium text-sm">{t("Retained member contributions")}</h3>{data.members?.length === 0 && <p className="text-sm text-muted-foreground">{t("No member readings have been assigned to this cycle yet.")}</p>}<div className="grid gap-2 sm:grid-cols-2">{data.members?.map(member => <MegaPigArchivedMember key={member.playerTag} member={member} onPlayer={openPlayer}/>)}</div>
    </div>}
    ...
    {view.mode === "player" && data && <details open={!data.history?.length} className="rounded border p-3"><summary className="cursor-pointer text-sm font-medium">{t("Saved source readings")}</summary><div className="mt-3 space-y-2"><p className="text-xs text-muted-foreground">{t("Cycle unconfirmed for these readings.")}</p>{!data.playerReadings?.length && <p className="text-sm text-muted-foreground">{t("No saved source readings for this tag yet.")}</p>}{data.playerReadings?.map(row => <article key={row.observation.id} className="rounded border p-3 space-y-1 min-w-0">{sourceBadge(row.observation.source)}<p className="text-sm font-medium break-words">{formatBrawlName(row.member.playerName, t("Player"))} <bdi dir="ltr" className="text-xs text-muted-foreground">{row.member.playerTag}</bdi></p><p className="text-sm">{t("Wins")}: {row.member.reportedWins === null ? t("Unknown") : number(row.member.reportedWins)} · {t("Tickets remaining")}: {row.member.reportedTicketsRemaining === null ? t("Unknown") : number(row.member.reportedTicketsRemaining)}</p><p className="text-xs text-muted-foreground">{t("First fetched")}: {dateTime(row.observation.firstFetchedAt)}</p><p className="text-xs text-muted-foreground">{t("Last fetched from source")}: {dateTime(row.observation.lastFetchedAt)}</p></article>)}</div></details>}
    ...
    <details className="text-xs text-muted-foreground"><summary className="cursor-pointer">{t("About this archive")}</summary><p className="mt-2">{t("Member results are kept after departure. Source readings do not confirm attendance or rewards; confirm final results from the game.")}</p></details>
  </section>;
}
