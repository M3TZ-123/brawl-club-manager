...
export function ClubRetention() {
  ...
  return <ClubIntelligencePanel title="Observed member retention" resource={resource}>{data && <>
    <p className="text-xs text-muted-foreground">{t("Based on recorded joins in the last 90 days.")}</p>
    <div className="grid gap-3 sm:grid-cols-2">{data.cohorts.map(cohort => <div key={cohort.days} className="rounded-lg bg-muted/40 p-3"><p className="text-sm">{t("Still present after {days} days", { days: number(cohort.days) })}</p><p className={`my-1 font-semibold ${cohort.rate === null ? "text-sm text-muted-foreground" : "text-2xl"}`}>{cohort.rate === null ? t("Not enough follow-up yet") : `${number(cohort.rate)}%`}</p><p className="text-xs text-muted-foreground">{t("{retained} of {eligible} observed stays", { retained: number(cohort.retained), eligible: number(cohort.eligible) })}</p></div>)}</div>
    ...
    <details className="text-xs text-muted-foreground"><summary className="cursor-pointer font-medium">{t("How retention is counted")}</summary><div className="mt-2 space-y-2"><p>{t("Recorded joining spells in the last 90 days. Returns start a separate spell; first observations are excluded from join cohorts.")}</p>
      {data.cohorts.map(cohort => <p key={cohort.days}>{t("Still present after {days} days", { days: number(cohort.days) })}: {t("{retained} retained / {eligible} eligible spells; {pending} awaiting follow-up; {excluded} uncertain or first-observed.", { retained: number(cohort.retained), eligible: number(cohort.eligible), pending: number(cohort.pending), excluded: number(cohort.excluded) })}</p>)}
      <p>{t("Dates reflect observations. Changes between roster checks can be missed; first observed is not an exact joining date.")}</p>
    </div></details>
