...
export function HistoryMemberCard({ member, isAdmin, onReview }: { member: MemberHistory; isAdmin: boolean; onReview: () => void }) {
  ...
  return <details className="rounded-lg border p-4" onToggle={event => setExpanded(event.currentTarget.open)}>
    <summary className="cursor-pointer"><span className="flex flex-wrap items-center justify-between gap-2"><bdi className="font-semibold">{member.player_name}</bdi><span className="text-xs text-muted-foreground">{t(member.is_current_member ? "Current" : "Former")}</span></span><bdi dir="ltr" className="text-xs text-muted-foreground">{member.player_tag}</bdi><span className="block mt-2 text-sm text-primary">{t(isAdmin ? "Details and notes" : "Details")}</span></summary>
    {expanded && <div className="mt-4 space-y-4"><dl className="grid grid-cols-2 gap-3 text-sm">
      ...
      <div><dt className="text-muted-foreground">{t("Recorded joins")}</dt><dd>{number(member.times_joined)}</dd></div>
      <div><dt className="text-muted-foreground">{t("Recorded departures")}</dt><dd>{number(member.times_left)}</dd></div>
    ...
    </dl>{isAdmin && <div><p className="text-sm font-medium">{t("Private notes")}</p><p className="whitespace-pre-wrap break-words text-sm text-muted-foreground">{member.notes || t("No private notes")}</p><Button variant="outline" className="mt-2" onClick={onReview}>{t("Review member")}</Button></div>}
