...
export function MemberAdministrationPanel({playerTag,isCurrent}:{playerTag:string;isCurrent?:boolean}){
  ...
  return <section className="space-y-4 border-t pt-5">
    <div className="space-y-1"><h3 className="font-semibold">{t("Dated history")}</h3><p className="text-xs text-muted-foreground">{t("Saved entries cannot be edited. Add a correction.")}</p></div>
    ...
    {!data?(loading?<p role="status">{t("Loading...")}</p>:null):<>
      ...
      {data.decisions.slice(0,visibleEntries).some(entry=>entry.kind==="follow_up")&&<p className="text-xs text-muted-foreground">{t("Historical record only. Schedule follow-ups in the note above.")}</p>}
