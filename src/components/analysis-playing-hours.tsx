...
export function AnalysisPlayingHours({ data }: { data: AnalysisResponse }) {
  ...
  return <section className="space-y-4 min-w-0" aria-label={t("Playing hours")}>
    <div className="flex flex-wrap items-center justify-between gap-2"><h2 className="text-lg font-semibold">{t(busiest.length ? "Hours with the most different members" : "When members are recorded playing")}</h2><bdi className="text-xs text-muted-foreground">{data.timeZone || "UTC"}</bdi></div>
    {busiest.length > 0 && <p className="text-xs text-muted-foreground">{t("Distinct members across this period, not simultaneous players.")}</p>}
    {!positive ? <p className="rounded-lg border border-dashed p-6 text-sm text-muted-foreground">{t("No recorded battles match this period and these filters. This does not prove inactivity.")}</p> : <>
      {busiest.length > 0 && <div className="grid gap-3 sm:grid-cols-3">
