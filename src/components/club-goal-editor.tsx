...
export function ClubGoalEditor({onSaved,onCancel}:{onSaved:(id:string)=>void;onCancel:()=>void}){
  ...
  return <form className="max-w-2xl rounded-lg border bg-card p-4 space-y-4" onSubmit={event=>{event.preventDefault();void save({action:"create_goal",title,metric,cycle,endsAt:cycle==="custom"?inputDate(end):null,target:Number(target)});}}>
    <fieldset disabled={busy} className="space-y-4"><h2 className="text-xl font-semibold">{t("Create a club goal")}</h2><p className="text-sm text-muted-foreground">{t("Starts when saved, with the current members. Target and dates stay fixed.")}</p>
    ...
    <details className="text-sm text-muted-foreground"><summary className="cursor-pointer">{t("How progress is measured")}</summary><div className="mt-2 space-y-2"><p>{t(metric==="trophies"?"Uses saved account trophy balances for the original members, not a sum of battle rewards. Departed members keep their latest known contribution.":"Counts original members with at least one observed battle during the goal. Missing battle logs can undercount participation.")}</p>
    <p>{t("The target, dates and roster cannot be edited later. Archive and create a new goal to change them.")}</p></div></details>
...
export function ClubGoalDetails({data,onArchived}:{data:PlanningResponse;onArchived:()=>void}){
  ...
  return <section className="border rounded-lg p-4 space-y-4"><h2 className="text-xl font-semibold">{goal.title}</h2><p className="text-lg font-semibold">{goal.progress===null?t("Unknown"):number(goal.progress)} / {number(goal.target)}</p>{goal.limited&&<p className="text-sm text-amber-600">{t("Progress may be incomplete")}</p>}
    <details><summary className="cursor-pointer font-medium">{t("Frozen roster: {count} members",{count:number(detail.members.length)})}</summary><div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{detail.members.map(m=><article key={m.playerTag} className="border rounded p-3 text-sm space-y-1"><p className="font-medium break-words">{m.playerName} {m.departed&&<span className="text-muted-foreground">· {t("Former member")}</span>}</p><p dir="ltr" className="text-xs text-muted-foreground">{m.playerTag}</p>{goal.metric==="trophies"?<p>{t("Baseline")}: {m.baselineTrophies===null?t("Unknown"):number(m.baselineTrophies)} · {t("Latest observed")}: {m.latestTrophies===null?t("Unknown"):number(m.latestTrophies)}</p>:<p>{t(m.participated?"Participation observed":"No participation observed")}</p>}{m.latestAt&&<p className="text-xs text-muted-foreground">{dateTime(m.latestAt)}</p>}{m.possibleGap&&<p className="text-amber-600">{t("Possible battle gap")}</p>}</article>)}</div></details>
