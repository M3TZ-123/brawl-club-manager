...
test('short history shows its actual comparison dates and keeps explanations and daily records collapsed', () => {
  ...
  assert.match(textContent(tree), /Partial period — since first record/);
...
test('trophy deltas retain positive, negative and zero values without showing minus zero for absent departures', () => {
  for (const [change, expected] of [[2878, '+2,878'], [-350, '-350'], [0, '0']]) {
    ...
    assert.doesNotMatch(textContent(tree), /Partial period — since first record/);
...
test('an unavailable optional report offers retry and is distinct from an incomplete history', () => {
  ...
  assert.doesNotMatch(textContent(tree), /complete roster record|Partial period — since first record/);
  ...
  for (const savedPoints of [[], points.slice(0, 1)]) {
    ...
    assert.match(textContent(pending), savedPoints.length ? /Waiting for a second complete roster record/ : /Not enough complete roster history yet/);
...
test('Arabic comparison labels and partial-history guidance are translated and dates remain isolated for RTL', () => {
  ...
  for (const label of ['تغيّر كؤوس النادي', 'فترة جزئية — منذ أول رصد', 'الأعضاء المشتركون', 'الأعضاء المنضمون', 'الأعضاء المغادرون', 'اليوم (UTC)', 'آخر رصد (بالتوقيت المحلي)']) assert.ok(text.includes(label), label);
  assert.doesNotMatch(text, /Club trophy changes|Partial period|Requested period|Last observed|Same members/);
