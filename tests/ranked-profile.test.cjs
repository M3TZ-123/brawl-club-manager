...
test('thirty valid profiles need zero RNT calls; sparse fallback starts after all profiles are fetched',async()=>{
 for(const missing of [[],['#P04','#P29']]){
  ...
  const service=loadTypeScript('src/lib/sync-service.ts',{'@/lib/supabase-admin':{supabaseAdmin:db},'@/lib/brawl-api':api,'@/lib/mega-pig-source-cache':{refreshMegaPigSource:async()=>{}}},{setTimeout:fn=>{fn();return 0;}});
