...
async function reportImage({report={},locale='en'}={}){
  ...
  const{ClubReportCard}=loadTypeScript('src/components/club-report-card.tsx',{...componentMocks,react:renderer.react,'@/components/locale-provider':{useI18n:()=>i18n},'@/lib/store':{useAppStore:selector=>selector({clubName:'Test club'})}},{document,URL:{createObjectURL:()=> 'blob:test',revokeObjectURL(){}},setTimeout:fn=>{fn();return 0;},fetch:async url=>{requests.push(url);throw new Error('Report images must use the supplied report without extra requests');}});
  ...
  let tree=await renderer.render(()=>ClubReportCard({report:value}));action(tree,locale==='ar'?translate('Download report image','ar'):'Download report image')();tree=await renderer.render(()=>ClubReportCard({report:value}));assert.doesNotMatch(textContent(tree),/Report image could not/);const link=elements(tree).find(e=>e.type==='a'&&e.props.download);assert.ok(link,'Generated image has an explicit browser download link');assert.equal(link.props.href,'blob:test');downloads.push(link.props);const preview=elements(tree).find(e=>e.type==='Image');return{drawn:drawn.map(r=>r.text),positions:drawn,context,downloads,canvas,requests,preview};
}
test('report image preserves unknown coverage and zero-observation win share without exposing private fields',async()=>{
  const image=await reportImage();assert.equal(image.downloads.length,1);assert.equal(image.canvas.width,1200);assert.equal(image.canvas.height,750);assert.equal(image.preview.props.width,1200);assert.equal(image.preview.props.height,750);assert.ok(image.drawn.includes('—'));assert.ok(image.drawn.includes('Trophy progress coverage: Unknown/30 members'));assert.doesNotMatch(image.drawn.join('\n'),/SECRET|0%|[Gg]oals/);assert.ok(image.positions.every(row=>row.y<750));
});
test('report image uses the supplied metrics and top gain without planning requests or a goals section',async()=>{
  const image=await reportImage({report:{summary:{totalMembers:30,totalTrophies:100000,weeklyBattles:10,weeklyWins:6,weeklyWinRate:60,trophyProgressKnownMembers:28},topGainers:[{playerName:'Alpha',trophyChange:42,notes:'SECRET'}]}});
  assert.ok(image.drawn.includes('Top observed gain: Alpha · +42'));assert.ok(image.drawn.includes('Trophy progress coverage: 28/30 members'));assert.ok(image.drawn.includes('60%'));assert.ok(image.drawn.includes('Public report · no private notes'));assert.deepEqual(image.requests,[]);assert.doesNotMatch(image.drawn.join('\n'),/[Gg]oals|SECRET/);
});
