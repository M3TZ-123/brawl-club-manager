-- Transaction-consistent snapshots, exported in bounded authenticated chunks.
BEGIN;
...
CREATE TABLE IF NOT EXISTS public.backup_chunks (
  ...
  payload text NOT NULL CHECK (octet_length(payload) <= 700000),
  byte_count integer NOT NULL CHECK (byte_count BETWEEN 1 AND 524288),
  sha256 text NOT NULL CHECK (sha256 ~ '^[0-9a-f]{64}$'),
  PRIMARY KEY(snapshot_id,table_name,chunk_index)
);
ALTER TABLE public.backup_snapshots ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.backup_chunks ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON public.backup_snapshots, public.backup_chunks FROM PUBLIC, anon, authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.backup_snapshots, public.backup_chunks TO service_role;
...
DECLARE
  v_allowed text[] := ARRAY['members','activity_log','club_events','member_history','settings','battle_history','player_tracking','brawler_snapshots','daily_stats','notifications','profiles','clubs','user_clubs','sync_runs','sync_leases','membership_change_events','notification_outbox','member_activity_state','player_brawler_state','member_reviews','admin_login_attempts','backup_snapshots','backup_chunks'];
...
CREATE OR REPLACE FUNCTION public.complete_backup_snapshot(p_snapshot_id uuid,p_artifact_sha256 text)
RETURNS boolean LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog AS $$
BEGIN
  IF p_artifact_sha256 IS NULL OR p_artifact_sha256 !~ '^[0-9a-f]{64}$' THEN RAISE EXCEPTION 'backup_artifact_hash_required' USING ERRCODE='22023'; END IF;
  UPDATE public.backup_snapshots SET status='complete',artifact_sha256=p_artifact_sha256,completed_at=clock_timestamp(),expires_at=clock_timestamp()+interval '30 days'
    WHERE id=p_snapshot_id AND (artifact_sha256 IS NULL OR artifact_sha256=p_artifact_sha256);
  IF NOT FOUND THEN RETURN false; END IF;
  DELETE FROM public.backup_chunks WHERE snapshot_id=p_snapshot_id;
  RETURN true;
END;
$$;
REVOKE ALL ON FUNCTION public.complete_backup_snapshot(uuid,text) FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION public.complete_backup_snapshot(uuid,text) TO service_role;
DO $$
DECLARE item text;
BEGIN
  FOREACH item IN ARRAY ARRAY['backup_snapshots','backup_chunks'] LOOP
    IF EXISTS(SELECT 1 FROM pg_publication_tables WHERE pubname='supabase_realtime' AND schemaname='public' AND tablename=item) THEN
