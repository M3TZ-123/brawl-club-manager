-- Coordinate backup capture with sync admission without holding a database
-- transaction open during external API requests. No cadence or data changes.
BEGIN;
...
BEGIN
  ...
  END IF;
  -- Admission and the durable lease handoff are atomic with respect to a
  -- snapshot's exclusive mutex, including before this lease becomes visible.
  IF NOT pg_catalog.pg_try_advisory_xact_lock_shared(184772931,3) THEN
    -- Terminal retries are read-only and can still return their exact outcome.
    IF p_idempotency_key IS NOT NULL THEN
      ...
      END IF;
    END IF;
    RETURN jsonb_build_object('acquired',false,'busy',true,'backup_busy',true);
  END IF;
...
REVOKE ALL ON FUNCTION public.acquire_sync_run(text,text,text,text,text) FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION public.acquire_sync_run(text,text,text,text,text) TO service_role;
