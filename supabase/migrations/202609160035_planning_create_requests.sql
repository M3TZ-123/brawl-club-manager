...
-- Keep only a digest and the created ID. Manual notes stay in their existing
-- private planning tables; receipts are retained with the bounded plans.
CREATE TABLE public.club_planning_create_requests (
  club_tag text NOT NULL CHECK (club_tag ~ '^#[A-Z0-9]{1,20}$'),
  request_id uuid NOT NULL,
  action text NOT NULL CHECK (action IN ('create_goal','save_event')),
  payload_sha256 bytea NOT NULL CHECK (octet_length(payload_sha256)=32),
  result_id uuid NOT NULL,
  created_at timestamptz NOT NULL DEFAULT clock_timestamp(),
  PRIMARY KEY(club_tag,request_id)
);
ALTER TABLE public.club_planning_create_requests ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON public.club_planning_create_requests FROM PUBLIC,anon,authenticated,service_role;
GRANT SELECT ON public.club_planning_create_requests TO service_role;

CREATE FUNCTION public.club_planning_create_once(p_club text,p_request_id uuid,p_payload jsonb)
RETURNS uuid LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog SET timezone='UTC' SET statement_timeout='8s' AS $$
DECLARE v_payload jsonb; v_event jsonb; v_entries jsonb; v_reason text; v_hash bytea;
  v_previous public.club_planning_create_requests; v_result uuid;
BEGIN
  IF p_request_id IS NULL OR p_club IS NULL OR p_club !~ '^#[A-Z0-9]{1,20}$'
    OR jsonb_typeof(p_payload) IS DISTINCT FROM 'object' OR octet_length(p_payload::text)>60000 THEN
    RAISE EXCEPTION 'invalid_planning' USING ERRCODE='22023';
  END IF;
  -- Canonicalize meaningful values before hashing. JSONB fixes object-key
  -- order; UTC timestamps and tag-sorted entries make equivalent retries match.
  IF p_payload->>'action'='create_goal' THEN
    IF EXISTS(SELECT 1 FROM jsonb_object_keys(p_payload) k WHERE k NOT IN ('action','title','metric','cycle','endsAt','target')) THEN
      RAISE EXCEPTION 'invalid_planning' USING ERRCODE='22023';
    END IF;
    v_payload:=jsonb_build_object('action','create_goal','title',btrim(p_payload->>'title'),
      'metric',p_payload->>'metric','cycle',p_payload->>'cycle','target',(p_payload->>'target')::integer,
      'endsAt',CASE WHEN p_payload->>'cycle'='custom' THEN (p_payload->>'endsAt')::timestamptz ELSE NULL END);
  ELSIF p_payload->>'action'='save_event' THEN
    IF EXISTS(SELECT 1 FROM jsonb_object_keys(p_payload) k WHERE k NOT IN ('action','id','version','event','entries','reason'))
      OR p_payload->'id' IS DISTINCT FROM 'null'::jsonb OR (p_payload->>'version')::integer IS DISTINCT FROM 0
      OR jsonb_typeof(p_payload->'event') IS DISTINCT FROM 'object' OR jsonb_typeof(p_payload->'entries') IS DISTINCT FROM 'array'
      OR jsonb_array_length(p_payload->'entries')>30 THEN RAISE EXCEPTION 'invalid_planning' USING ERRCODE='22023'; END IF;
    v_event:=(p_payload->'event')||jsonb_build_object('title',btrim(p_payload->'event'->>'title'),
      'cycleLabel',btrim(p_payload->'event'->>'cycleLabel'),'notes',btrim(coalesce(p_payload->'event'->>'notes','')),
      'startsAt',(p_payload->'event'->>'startsAt')::timestamptz,'endsAt',(p_payload->'event'->>'endsAt')::timestamptz,
      'teamSize',(p_payload->'event'->>'teamSize')::integer,'ticketAllowance',(p_payload->'event'->>'ticketAllowance')::integer);
    IF EXISTS(SELECT 1 FROM jsonb_array_elements(p_payload->'entries') e WHERE jsonb_typeof(e) IS DISTINCT FROM 'object') THEN
      RAISE EXCEPTION 'invalid_planning' USING ERRCODE='22023';
    END IF;
    SELECT coalesce(jsonb_agg(e||jsonb_build_object('playerTag',btrim(e->>'playerTag'),'team',(e->>'team')::integer,
      'wins',(e->>'wins')::integer,'ticketsRemaining',(e->>'ticketsRemaining')::integer,
      'observedAt',(e->>'observedAt')::timestamptz,'notes',btrim(coalesce(e->>'notes',''))) ORDER BY e->>'playerTag'),'[]')
      INTO v_entries FROM jsonb_array_elements(p_payload->'entries') e;
    v_reason:=btrim(coalesce(p_payload->>'reason',''));
    v_payload:=jsonb_build_object('action','save_event','event',v_event,'entries',v_entries,'reason',v_reason);
  ELSE RAISE EXCEPTION 'invalid_planning' USING ERRCODE='22023'; END IF;
  v_hash:=sha256(convert_to(v_payload::text,'UTF8'));
  PERFORM pg_advisory_xact_lock(hashtextextended(p_club||':'||p_request_id::text,3501));
  SELECT * INTO v_previous FROM public.club_planning_create_requests WHERE club_tag=p_club AND request_id=p_request_id;
  IF FOUND THEN
    IF v_previous.payload_sha256 IS DISTINCT FROM v_hash THEN
      RAISE EXCEPTION 'planning_request_changed' USING ERRCODE='40001';
    END IF;
    RETURN v_previous.result_id;
  END IF;
  -- Validation and cohort/event locks remain in the original functions.
  -- A failed create rolls back both its data and its receipt in this transaction.
  IF v_payload->>'action'='create_goal' THEN
    v_result:=public.club_planning_create_goal(p_club,v_payload->>'title',v_payload->>'metric',v_payload->>'cycle',
      (v_payload->>'endsAt')::timestamptz,(v_payload->>'target')::integer);
  ELSE
    v_result:=public.club_planning_save_event(p_club,NULL,0,v_event,v_entries,v_reason);
  END IF;
  INSERT INTO public.club_planning_create_requests(club_tag,request_id,action,payload_sha256,result_id)
    VALUES(p_club,p_request_id,v_payload->>'action',v_hash,v_result);
  RETURN v_result;
END $$;
REVOKE ALL ON FUNCTION public.club_planning_create_once(text,uuid,jsonb) FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION public.club_planning_create_once(text,uuid,jsonb) TO service_role;


-- Preserve the existing backup capture definition, adding only the receipt table.
