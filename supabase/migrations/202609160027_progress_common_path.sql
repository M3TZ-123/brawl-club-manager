-- Avoid repeating optional equipment normalization when a fetched value exactly
-- matches validated stored data. Partial/new data retains the025 preservation path.
-- No data writes, retention changes, new tables, or public permissions are added.
BEGIN;
...
DECLARE
  ...
  v_previous public.player_ranked_history%ROWTYPE; v_merged jsonb; v_saved jsonb; v_candidate jsonb; v_exists boolean; v_kind text;
BEGIN
  ...
  FOR v_item IN SELECT value FROM jsonb_array_elements(p_payload->'brawlers') LOOP
    ...
    v_saved:=to_jsonb(v_brawler);
    -- Exact equality is safe for already validated saved values. In particular,
    -- array containment must not be used: [] must still clear reported entries.
    IF FOUND AND ROW(v_item->>'brawler_name',(v_item->>'power_level')::integer,(v_item->>'trophies')::integer,(v_item->>'rank')::integer)
        IS NOT DISTINCT FROM ROW(v_brawler.brawler_name,v_brawler.power_level,v_brawler.trophies,v_brawler.rank)
      AND (v_input IS NULL OR jsonb_typeof(v_input)='object')
      AND NOT EXISTS(SELECT 1 FROM jsonb_each(CASE WHEN jsonb_typeof(v_input)='object' THEN v_input ELSE '{}'::jsonb END) supplied
        WHERE supplied.value IS DISTINCT FROM v_saved->supplied.key) THEN
      CONTINUE;
    END IF;
    ...
    FOREACH v_key IN ARRAY ARRAY['gadgets','star_powers','gears','hyper_charges'] LOOP
      v_value:=CASE WHEN jsonb_typeof(v_input->v_key)='array' AND v_input->v_key=v_saved->v_key
        THEN v_input->v_key ELSE public.sync_progress_equipment(v_input->v_key) END;
      IF v_value IS NOT NULL THEN
        v_merged:=CASE WHEN v_value=v_saved->v_key THEN v_value
          ELSE public.sync_merge_progress_equipment(v_saved->v_key,v_value) END;
      ...
      END IF;
    END LOOP;
    v_value:=CASE WHEN jsonb_typeof(v_input->'skin')='object' AND v_input->'skin'=v_brawler.skin
      THEN jsonb_build_array(v_brawler.skin) ELSE public.sync_progress_equipment(jsonb_build_array(v_input->'skin')) END;
    IF v_value IS NOT NULL THEN
      v_merged:=CASE WHEN v_value->0=v_brawler.skin THEN v_value
        ELSE public.sync_merge_progress_equipment(jsonb_build_array(v_brawler.skin),v_value) END;
    ...
    END IF;
    v_candidate:=coalesce(v_saved,'{}')||v_optional||jsonb_build_object(
